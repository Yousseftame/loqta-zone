 ...
 *
 * avgWinningBid is derived client-side from totalRevenue / endedAuctions
...
function parseTopAuction(raw: any) {
  return {
    auctionId:         raw.auctionId         ?? "",
    auctionNumber:     raw.auctionNumber     ?? 0,
    productTitle:      raw.productTitle      ?? "",
    totalBids:         raw.totalBids         ?? 0,
    totalParticipants: raw.totalParticipants ?? 0,
    winningBid:        raw.winningBid        ?? 0,
    actualPrice:       raw.actualPrice       ?? 0,
    actualProfit:      raw.actualProfit      ?? 0,
    margin:            raw.margin            ?? 0,
  ...
  const refetch = useCallback(async () => {
    ...
    try {
      ...
      if (topSnap.exists()) {
        ...
        setTopAuctions({
          byBids:          (d.byBids         ?? []).map(parseTopAuction),
          byParticipants:  (d.byParticipants ?? []).map(parseTopAuction),
          byWinningBid:    (d.byWinningBid   ?? []).map(parseTopAuction),
          financialReport: (d.financialReport ?? []).map(parseTopAuction),
