 ...
 *
 * Full-screen celebration overlay shown to ALL participants when an auction ends
 * and a winner is determined.
 *
 * Props:
 *  - winnerName    : display name of the winning bidder
 *  - winningBid    : the final winning amount in EGP
 *  - isWinner      : true if the current logged-in user is the winner
        ...
        <div className="wc-content">
          ...
          <div className={`wc-text-block ${textVisible ? "wc-in" : ""}`}>
            <div className={`wc-headline ${isWinner ? "wc-you-won" : ""}`}>
              {isWinner ? "You Won! 🎉" : "We Have a Winner!"}
            </div>

            {productTitle && <div className="wc-product">{productTitle}</div>}
            ...
            {isWinner && (
              <div className="wc-congrats">
                Congratulations! You secured this auction with the highest bid.
              </div>
            )}
          </div>
          ...
          <div className={`wc-bid-block ${bidVisible ? "wc-in" : ""}`}>
            <div className="wc-bid-label">Winning Bid</div>
...
.wc-trophy {
  ...
  animation: wc-float 3s ease-in-out 1.2s infinite;
  filter: drop-shadow(0 0 30px rgba(201,169,110,0.5));
}
.wc-trophy.wc-in {
  animation: wc-pop 0.7s cubic-bezier(0.34,1.56,0.64,1) both,
             wc-float 3s ease-in-out 1.9s infinite;
}
...
.wc-headline.wc-you-won {
  background: linear-gradient(135deg, #c9a96e 0%, #fde68a 40%, #c9a96e 80%);
